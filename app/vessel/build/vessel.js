...
*/
	...
	function trapezoidCalculation(xbase0, xbase1, xtop0, xtop1, ybase, ytop) {
		...
		let Iy = steiner(Iyrt1, Art1, xcrt1, xc) + steiner(Iyrec, Arec, xcrec, xc) + steiner(Iyrt2, Art2, xcrt2, xc)
...
*/
	...
	Object.assign(Ship.prototype, {
		...
		getWeight: function (shipState) {
			...
			components.push(this.structure.getWeight(this.designState))
		...
		subtractFuelMass: function (mass, shipState) {
			...
			if (mass <= totalFuel) {
				// if yes, subtract mass in the same proportion
				for (var tk = 0; tk < tankEntr.length; tk++) {
					var tkId = tankEntr[tk][0]
					shipState.objectCache[tkId].state.fullness -= mass / fuelCap
				}
			} else {
				// if not, make tanks empty
				mass -= totalFuel
				for (var tk = 0; tk < tankEntr.length; tk++) {
					var tkId = tankEntr[tk][0]
	...
	*/
		...
		calculateAttributesAtDraft: (function () {
			function levelCalculation(
				...
				if (wlc.minX !== null && !isNaN(wlc.minX) && wlc.minX <= prev.minX) lev.minX = wlc.minX
				else lev.minX = prev.minX
				if (wlc.maxX !== null && !isNaN(wlc.maxX) && wlc.maxX >= prev.maxX) lev.maxX = wlc.maxX
				else lev.maxX = prev.maxX
				if (wlc.minY !== null && !isNaN(wlc.minY) && wlc.minY <= prev.minY) lev.minY = wlc.minY
				else lev.minY = prev.minY
				if (wlc.maxY !== null && !isNaN(wlc.maxY) && wlc.maxY >= prev.maxY) lev.maxY = wlc.maxY
				else lev.maxY = prev.maxY
				...
				for (let j = 0; j < sts.length - 1; j++) {
					let port = patchColumnCalculation(sts[j], sts[j + 1], prev.z, z, -prwl[j], -wl[j], -prwl[j + 1], -wl[j + 1])
					calculations.push(port)
					let star = patchColumnCalculation(sts[j], sts[j + 1], prev.z, z, prwl[j], wl[j], prwl[j + 1], wl[j + 1])
				...
				//End caps:
				if (lev.minXwp <= sts[0]) lev.As += hull.stationCalculation(lev.minXwp, z)["A"]
				if (lev.maxXwp >= sts[sts.length - 1]) lev.As += hull.stationCalculation(lev.maxXwp, z)["A"]

				//center of volume below z (some potential for accumulated rounding error when calculating an accumulated average like this):
				lev.Cv = Vectors.scale(Vectors.add(Vectors.scale(prev.Cv, prev.Vs), Vectors.scale(Cv, C.V)), 1 / (lev.Vs || 2))
...
*/
	...
	function WaveCreator(defList, waveDuration = 3600) {
		// wave creator constructor
		...
		this.defList = defList || [
			// list wave definitions
			[0.6, 2.25, 180],
			[1.1, 1.75, 150]
		]
		this.setRandom = function () {
			// set a wave definition randomly
	...
	Object.defineProperties(WaveMotion.prototype, {
		...
		verticalMotion: StateModule.prototype.memoized(function () {
			...
			var sectional_hydro_damping = 2 * Math.sin(0.5 * this.coefficients.wave_number * Breadth * Math.pow(this.coefficients.alpha, 2)) * Math.exp(-this.coefficients.wave_number * this.floatState.T * Math.pow(this.coefficients.alpha, 2))
			...
			var f = Math.sqrt(a + b)
			var eta = 1 / Math.sqrt(Math.pow(1 - 2 * this.coefficients.wave_number * this.floatState.T * Math.pow(this.coefficients.alpha, 2), 2) + Math.pow(Math.pow(sectional_hydro_damping, 2) / (this.coefficients.wave_number * Breadth * Math.pow(this.coefficients.alpha, 2)), 2))
			...
			var G = this.coefficients.smith_factor * f * (24 / (Math.pow(this.coefficients.eff_wave_number * this.floatState.LWL, 2) * this.floatState.LWL)) * (Math.sin((this.coefficients.eff_wave_number * this.floatState.LWL) / 2) - ((this.coefficients.eff_wave_number * this.floatState.LWL) / 2) * Math.cos((this.coefficients.eff_wave_number * this.floatState.LWL) / 2))
		...
		rollAmp: StateModule.prototype.memoized(function () {
			...
			var b_44_0 = (this.rho * A_0 * Math.pow(this.floatState.BWL, 2) * a0 * Math.exp(b0 * Math.pow(this.coefficients.encounter_frequency, -1.3)) * Math.pow(this.coefficients.encounter_frequency, d0)) / Math.sqrt(this.floatState.BWL / (2 * this.g))
			...
			var b_44_1 = (this.rho * A_1 * Math.pow(B_1, 2) * a1 * Math.exp(b1 * Math.pow(this.coefficients.encounter_frequency, -1.3)) * Math.pow(this.coefficients.encounter_frequency, d1)) / Math.sqrt(B_1 / (2 * this.g))
			...
			if (this.wavCre.waveDef.heading == 90 || this.wavCre.waveDef.heading == 270) {
				excitation_frequency = Math.sqrt((this.rho * Math.pow(this.g, 2) * b_44_0) / this.coefficients.encounter_frequency) * (this.delta + damping_ratio * (1 - this.delta)) * this.floatState.LWL
			} else {
				...
				D = 2 * damping_ratio * Math.sin(0.5 * this.delta * this.floatState.LWL * this.coefficients.eff_wave_number) * Math.sin(0.5 * (1 - this.delta) * this.floatState.LWL * this.coefficients.eff_wave_number) * Math.cos(0.5 * this.floatState.LWL * this.coefficients.eff_wave_number)
	...
	function Positioning(ship, states, path) {
		...
		this.advanceShip = function (timeStep) {
			// calculate advanced distance during one time step
			var remVec, remDist
			...
			while (0 < advDist) {
				...
				if (advDist <= remDist) {
					...
					advDist = 0
				} else if (path[this.legState.leg + 2] !== undefined) {
					// trip has another leg
	...
	function FuelConsumption(ship, states, powerPlant) {
		...
		this.setAuxPower = function (Paux) {
			...
			if (typeof this.states.discrete.AuxPower === "undefined") {
				// Ps and Paux in W each
	...
	Object.defineProperties(
		FuelConsumption.prototype,
		{
			consumptionRate: StateModule.prototype.memoized(function () {
				...
				function shareLoad(system, load) {
					...
					if (load <= triggerRatio * partCapac) {
						// if not all engines are loaded above trigger ratio, load them according to trigger rate ceil
						var capSum = 0
						loads.fill(0)
						for (var eng = 0; eng < engCapac.length; eng++) {
							capSum += engCapac[eng]
							if (load <= triggerRatio * capSum) {
								// if engines can support load
								for (i = 0; i <= eng; i++) {
									// distribute load proportionally to engines' capacities
									loads[i] = load / capSum
								}
								break
							}
						}
					} else if (triggerRatio * partCapac < load && load <= totalCapac) {
						// if all engines are loaded above trigger ratio, make them all have same load %
				...
				var consumptionRate
				if (typeof this.powerPlant.auxiliary === "object") {
					// calculate results for vessels which have main and auxiliary power systems
					...
					// calculate results for vessels which have only one power system
	...
	Object.defineProperties(HullResistance.prototype, {
		coefficients: StateModule.prototype.memoized(function () {
			...
			var wa = this.floatState.LWL * (2 * T + this.floatState.BWL) * Math.pow(this.floatState.Cm, 0.5) * (0.453 + 0.4425 * this.floatState.Cb - 0.2862 * this.floatState.Cm - (0.003467 * this.floatState.BWL) / T + 0.3696 * this.floatState.Cwp) + (2.38 * abt) / this.floatState.Cb // wetted area
			...
			var k = 0.93 + 0.487118 * c14 * Math.pow(this.floatState.BWL / this.floatState.LWL, 1.06806) * Math.pow(T / this.floatState.LWL, 0.46106) * Math.pow(this.floatState.LWL / lr, 0.121563) * Math.pow(Math.pow(this.floatState.LWL, 3) / this.floatState.Vs, 0.36486) * Math.pow(1 - this.floatState.Cp, -0.604247) // form factor
		...
		calmResistance: StateModule.prototype.memoized(function () {
			// N, total hull resistance in calm waters
			...
			var ie = 1 + 89 * Math.exp(-Math.pow(this.floatState.LWL / this.floatState.BWL, 0.80856) * Math.pow(1 - this.floatState.Cwp, 0.30484) * Math.pow(1 - this.floatState.Cp - 0.0225 * (this.coefficients.lcb / 100), 0.6367) * Math.pow(this.coefficients.lr / this.floatState.BWL, 0.34574) * Math.pow(100 * (this.floatState.Vs / Math.pow(this.floatState.LWL, 3)), 0.16302))
		...
		efficiency: StateModule.prototype.memoized(function () {
			...
			if (this.propeller.noProps === 1) {
				t = (0.25014 * Math.pow(this.floatState.BWL / this.floatState.LWL, 0.28956) * Math.pow(Math.pow(this.floatState.BWL * this.coefficients.T, 0.5) / this.propeller.D, 0.2624)) / Math.pow(1 - this.floatState.Cp + 0.0225 * this.coefficients.lcb, 0.01762) + 0.0015 * this.cstern
			} else if (this.propeller.noProps === 2) {
