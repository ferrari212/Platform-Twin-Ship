...
// HullGeometry is a ES6 version of HullSideGeometry
// HullSideGeometry is depricated
class HullGeometry extends THREE.PlaneBufferGeometry {
	constructor(stations, waterlines, table) {
		super(undefined, undefined, stations.length - 1, waterlines.length - 1)
...
Object.assign(Hull3D.prototype, {
	...
	update: function () {
		...
		this.hGeom = new HullGeometry(stations, waterlines, table)
		...
		if (!this.hMat) {
			...
			this.hMat = new THREE.ShaderMaterial({
				...
				fragmentShader: commonDecl + phong.fragmentShader.replace("vec4 diffuseColor = vec4( diffuse, opacity );", "vec4 diffuseColor = vec4( (vZ>wlThreshold)? aboveWL.rgb : belowWL.rgb, opacity );").replace("#define PHONG", ""),
